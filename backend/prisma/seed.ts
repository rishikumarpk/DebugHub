...
async function main() {
  const challenges = [
    ...
    // Adding 35 more to reach 50 total challenges
    ...Array.from({ length: 35 }).map((_, i) => ({
      date: new Date(new Date('2026-03-08T00:00:00Z').getTime() + i * 24 * 60 * 60 * 1000),
      language: i % 2 === 0 ? 'python' : 'javascript',
      bugType: ['SYNTAX', 'LOGICAL', 'EDGE_CASE', 'TYPE_ERROR', 'PERFORMANCE'][i % 5],
      difficulty: ['EASY', 'MEDIUM', 'HARD'][i % 3],
      context: `Autogenerated mock challenge #${16 + i} for demonstration`,
      expectedOutput: 'mock_success',
      hint1: 'Check the syntax basics.',
      hint2: 'Review the logic flow.',
      hint3: 'Fix the obvious error.',
      correctCode: i % 2 === 0 ? `print('mock_success')` : `console.log('mock_success');`,
      buggyCode: i % 2 === 0 ? `print('mock_successs')` : `console.log('mock_successs');`,
    }))
  ]
