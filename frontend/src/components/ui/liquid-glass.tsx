...
const GlassEffect: React.FC<GlassEffectProps> = ({
    ...
    const content = (
        <div
            className={`relative flex font-semibold overflow-hidden text-[#E0E0E0] cursor-pointer transition-all duration-700 ${className}`}
            style={glassStyle}
        >
            {/* Glass Layers */}
            <div
    ...
    return href ? (
        <a href={href} target={target} rel="noopener noreferrer" className="block w-max">
            {content}
        </a>
    ) : (
        content
